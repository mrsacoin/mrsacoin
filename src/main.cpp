...
unsigned int nWorkTargetSpacing = 360;                  // 360 sec block spacing for PoW
...
const string strMessageMagic = "MrsaCoin Signed Message:\n";
...
void SyncWithWallets(const CTransaction& tx, const CBlock* pblock, bool fUpdate, bool fConnect)
{
    if (!fConnect)
    {
        // MrsaCoin: wallets need to refund inputs when disconnecting coinstake
        if (tx.IsCoinStake())
        {
            BOOST_FOREACH(CWallet* pwallet, setpwalletRegistered)
                if (pwallet->IsFromMe(tx))
                        ...
                        bool* pfMissingInputs)
{
    ...
    // MrsaCoin: coinstake is also only valid in a block, not as a loose transaction
    if (tx.IsCoinStake())
...
// MrsaCoin: find last block index up to pindex
    ...
    const CBlockIndex* pindexBlock, bool fBlock, bool fMiner)
{
    ...
    if (!IsCoinBase())
    {
        ...
        for (unsigned int i = 0; i < vin.size(); i++)
        {
            ...
            // MrsaCoin: check transaction timestamp
...
bool CBlock::DisconnectBlock(CTxDB& txdb, CBlockIndex* pindex)
{
    ...
    // MrsaCoin: clean up wallet after disconnecting coinstake
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    if (IsProofOfStake())
    {
        // MrsaCoin: coin stake tx earns reward instead of paying fee
        uint64_t nCoinAge;
    ...
    // MrsaCoin: track money supply and mint amount info
...
// MrsaCoin: total coin age spent in transaction, in the unit of coin-days.
...
// MrsaCoin: total coin age spent in block, in the unit of coin-days.
...
bool CBlock::AddToBlockIndex(unsigned int nFile, unsigned int nBlockPos, const uint256& hashProofOfStake)
{
    ...
    // MrsaCoin: compute chain trust score
    pindexNew->nChainTrust = (pindexNew->pprev ? pindexNew->pprev->nChainTrust : 0) + pindexNew->GetBlockTrust();

    // MrsaCoin: compute stake entropy bit for stake modifier
    if (!pindexNew->SetStakeEntropyBit(GetStakeEntropyBit()))
        return error("AddToBlockIndex() : SetStakeEntropyBit() failed");

    // MrsaCoin: record proof-of-stake hash value
    pindexNew->hashProofOfStake = hashProofOfStake;

    // MrsaCoin: compute stake modifier
...
bool CBlock::AcceptBlock()
{
    ...
    // MrsaCoin: check pending sync-checkpoint
...
bool ProcessBlock(CNode* pfrom, CBlock* pblock)
{
    ...
    // MrsaCoin: ask for pending sync-checkpoint if any
    ...
    if (!mapBlockIndex.count(pblock->hashPrevBlock))
    {
        ...
        if (pfrom)
        {
            ...
            // MrsaCoin: getblocks may not obtain the ancestor block rejected
    ...
    // MrsaCoin: if responsible for sync-checkpoint send it
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "MrsaCoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "MrsaCoin: the coin for equality!!!";
        CTransaction txNew;
        txNew.nTime = 1482288931;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1482288931;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = 1539485;
        if(fTestNet)
        {
            block.nNonce = 0;
        }
        ...
        //// debug print
        assert(block.hashMerkleRoot == uint256("6a09699d0d7aa6113c779c2a602b3467c044591ca0efd63abf1549834c8cbf8d"));
        // block.print();
        ...
        // MrsaCoin: initialize synchronized checkpoint
...
unsigned char pchMessageStart[4] = { 0xc3, 0xc1, 0xfa, 0xfc };
        ...
        BOOST_FOREACH(const CInv& inv, vInv)
        {
            ...
            if (inv.type == MSG_BLOCK)
            {
                ...
                if (mi != mapBlockIndex.end())
                {
                    ...
                    {
                        // MrsaCoin: send latest proof-of-work block to allow the
    ...
    else if (strCommand == "getblocks")
    {
        ...
        for (; pindex; pindex = pindex->pnext)
        {
            if (pindex->GetBlockHash() == hashStop)
            {
                ...
                // MrsaCoin: tell downloading node about the latest block if it's
